...
### 계정
- 회원가입, 로그인 (신규 가입은 관리자 승인 후 로그인 가능)
- 설정 화면에서 이름, 이메일, 비밀번호 수정 (이메일/비밀번호 변경 시 현재 비밀번호 확인)
- 관리자 페이지(`/admin`): 승인, 거절(삭제), 사용 중지/재개, 계정별 AI 기능 허용/차단, 관리자 지정/해제
- GPU를 쓰는 AI 기능은 계정별 허용제 (기본 꺼짐, 서버 부하 관리). CLI: `php spark user:set <email> admin|user|approve|block|ai-on|ai-off`

### 공유
- Send to X: X 새 글 작성 창에 본문을 채우고 파일을 첨부한 상태까지 만든다(게시는 수동). 크롬 확장 `extension/` 필요, 없으면 본문만 채우고 파일은 다운로드
- 30분짜리 서명 링크 `/share/{id}/{exp}/{hmac}`에만 x.com CORS 허용
...
- 회전/반전: 90도, 180도, -90도 회전과 좌우·상하 반전 (가장 먼저 적용, 이후 좌표는 회전된 화면 기준)
- 자막: 최대 2레이어, 템플릿 15종(외곽선/굵은 외곽선/회색 외곽선/그림자/반투명 박스/검정 박스/흰 박스/노란 강조/번짐+외곽선/번짐/노랑 굵은 외곽선/검정 글자/흰 반투명 박스/큰 그림자/네온), 문장별 독립 스타일(디자인/폰트/크기/색/위치), 타임라인 자막 트랙 2줄에서 구간 드래그(추가/이동/길이 조절), 폰트/크기/색/위치
- 워터마크: 9분할 위치 프리셋과 드래그, 크기/색/불투명도/스타일 4종
- 폰트 14종 (SIL OFL 7종 + 한글 무료 폰트 6종 + M PLUS Rounded 1c)
